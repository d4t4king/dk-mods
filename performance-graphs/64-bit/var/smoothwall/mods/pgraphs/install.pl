...
my $modbase = '/var/smoothwall/mods/pgraphs';
my $mod = '/var/smoothwall/mods/pgraphs/modfiles';
my $storebkp = '/var/smoothwall/mods/pgraphs/backup';

print "Installing new files ...\n";

system ("/bin/mkdir -p $modbase/httpd/cgi-bin/");
system ("/bin/mkdir -p $modbase/httpd/html/help/");
system ("/bin/mkdir -p $modbase/usr/lib/smoothwall/langs/");
system ("/bin/mkdir -p $modbase/usr/lib/smoothwall/menu/1000_About/");

$dir = $modbase . '/httpd/cgi-bin';
&install ('pgraphs.cgi', $dir, $mod);

$dir = $modbase . '/httpd/cgi-bin';
&install ('.htaccess', $dir, $mod);

$dir = $modbase . '/httpd/html/help';
&install ('pgraphs.cgi.html.en', $dir, $mod);

$dir = $modbase . '/usr/lib/smoothwall/langs';
&install ('en.pl', $dir, $mod);

$dir = $modbase . '/usr/lib/smoothwall/langs';
&install ('alertboxes.en.pl', $dir, $mod);

$dir = $modbase . '/usr/lib/smoothwall/langs';
&install ('glossary.en.pl', $dir, $mod);

$dir = $modbase . '/usr/lib/smoothwall/menu/1000_About';
...
### >>> if not exists (/usr/bin/fping) then
$dir = '/usr/bin';
...
`chmod 755 $modbase/httpd/cgi-bin/pgraphs.cgi`;
