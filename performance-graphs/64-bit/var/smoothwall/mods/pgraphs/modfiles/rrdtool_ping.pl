...
my $far_red_address = '/var/smoothwall/red/remote-ipaddress';
...
if ( "$targets[4]" eq 'automatic') {
	if ( -r $far_red_address ) {
                open(FILE, $far_red_address );
                while (<FILE>) {
                        chomp;
                        $targets[4] = $_;
                        print "for #4 automatic replaced by $_ \n";
		}
                close FILE;
        }
}
